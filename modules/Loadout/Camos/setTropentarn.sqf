...
if (uniform player in possCamoUnif) then
{
	...
	{
		if (uniform player in possCamoUnif && !("textures\Bundeswehr\Tropentarn\Infantry\clothing1_co.paa" in getObjectTextures player)) then
		{
			player setObjectTextureGlobal [0,"textures\Bundeswehr\Tropentarn\Infantry\clothing1_co.paa"];
		};
	}];
};